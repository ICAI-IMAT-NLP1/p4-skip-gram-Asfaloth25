...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        noise_words: torch.Tensor = torch.multinomial(
            noise_dist,
            num_samples=n_samples * batch_size,
            replacement=True,
        )
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        # we need to perform dot products between 
        dot_product = (input_vectors * output_vectors).sum(dim=1)
        out_loss = nn.functional.logsigmoid(dot_product)

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_loss = nn.functional.logsigmoid((
                input_vectors.unsqueeze(dim=1) * noise_vectors
                ).sum(dim=2)
            ).sum(dim=1)

        # Return the negative sum of the correct and noisy log-sigmoid losses, averaged over the batch
        # TODO
        return - (noise_loss.mean(dim=0) + out_loss.mean(dim=0))